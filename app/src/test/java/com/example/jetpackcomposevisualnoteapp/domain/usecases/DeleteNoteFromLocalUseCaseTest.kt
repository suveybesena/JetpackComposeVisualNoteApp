...
class DeleteNoteFromLocalUseCaseTest {
    @get:Rule
    ...
    private lateinit var deleteNoteFromLocalUseCase: DeleteNoteFromLocalUseCase
    ...
    fun setup() {
        ...
        deleteNoteFromLocalUseCase = DeleteNoteFromLocalUseCase(fakeRepository, testDispatcher)
    }

    @Test
    fun deleteNote_ShouldReturnLoadingThenResource_collectIndexed() {
        runBlockingTest {
            val date = System.currentTimeMillis()
            val noteDetail = NoteModel("www.test.com", date, "Title", "Desc", 1, 1,"edited", 1)
            deleteNoteFromLocalUseCase.invoke(noteDetail).collectIndexed { index, value ->
