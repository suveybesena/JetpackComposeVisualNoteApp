...
class UpdateNoteFromLocalUseCaseTest {
    @get:Rule
    ...
    private lateinit var updateNoteFromLocalUseCase: UpdateNoteFromLocalUseCase
    ...
    fun setup() {
        ...
        updateNoteFromLocalUseCase = UpdateNoteFromLocalUseCase(fakeRepository, testDispatcher)
    }

    @Test
    fun updateNote_ShouldReturnLoadingThenResource_collectIndexed() {
        runBlockingTest {
            val date = System.currentTimeMillis()
            val noteDetail = NoteModel("www.test.com", date, "Title", "Desc", 1, 1, "edited", 1)
            updateNoteFromLocalUseCase.invoke(noteDetail).collectIndexed { index, value ->
